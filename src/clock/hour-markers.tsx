...
function getDotConfig({ size = 1.5, depth = 73.5, split = 24, skip_current = true }) {
  return { size, depth, split, skip_current }
}

function getTextConfig({ depth = 80, split = 4 }) {
  return { depth, split }
}
    ...
    <g id="hour-markers" fill="#fff" stroke="#000">
      {[...Array(24)].map((_, _h) => {
        ...
        if (isCurrent || isPartOfSplit(_h, _text.split)) {
          ...
          text = (
            <g className={Cn(styles.hourMarkerText, {[styles.currentHour]: isCurrent})} style={{transform:`rotate(${rotation}deg) translate(0, ${isCurrent ? depth : _text.depth}px)`}}>
        ...
        if (!(_dot.skip_current && isCurrent) && isPartOfSplit(_h, _dot.split)) {
          const translate = `translate(0, ${_dot.depth}px)`
          dot = <circle
